...
class UniVLG(nn.Module):
    ...
    def process_pointcloud_in_chunks(self, pointcloud, featurecloud, attention_mask, target_sentence, chunk_size=4096):
        """
        ...
        for chunk_idx in range(num_chunks):
            ...
            chunk_attention_mask = self.build_knn_attention_mask(chunk_points, k=32)
    ...
    def process_point_cloud_and_text_with_qwen(self, feature_cloud, pointcloud, attention_mask, target_sentence):
        """
        ...
        Args:
            ...
            attention_mask: Binary attention mask for points [B, N, N]
        ...
        conversation = [
            {"role": "system", "content": "You are an advanced 3D scene understanding assistant. Your goal is to analyze the 3D scene according to the text."},
            {"role": "user", "content": f"Analyze this 3D scene and find: {target_sentence}"}
        ]
        
        text_inputs = self.qwen_processor.tokenizer(
            self.qwen_processor.tokenizer.apply_chat_template(
                conversation, 
                tokenize=False, 
        ...
        # Create a new attention mask that allows:
        # 1. Text tokens attend to text tokens (causal mask for text)
        # 2. Point tokens attend to other points according to KNN attention_mask
        # 3. Point tokens attend to all text tokens (text is contextual information)
        # 4. Text tokens do not attend to point tokens (to maintain causal property)
        
        # Start with a causal mask for text tokens
        combined_mask = torch.zeros((batch_size, total_length, total_length), device=device)
        
        # Create causal mask for text tokens (lower triangular)
        text_mask = torch.tril(torch.ones((text_length, text_length), device=device))
        
        # For each batch
        for b in range(batch_size):
            # 1. Text-to-text attention (causal)
            combined_mask[b, :text_length, :text_length] = text_mask
            
            # 2. Point-to-point attention (from KNN)
            combined_mask[b, text_length:, text_length:] = attention_mask[b]
            
            # 3. Point-to-text attention (points can see all text)
            combined_mask[b, text_length:, :text_length] = 1.0
        
        # Convert to attention mask format (1 = attend, 0 = mask)
        combined_mask = combined_mask.to(torch.float16)
        
        # We'll need to bypass the normal image processing pathway
        # and directly inject our point token embeddings
        
        # Get text embeddings from Qwen's embedding layer
        ...
        """No longer need this, because Qwen2.5 has the patch merger, and I have used it in visual"""
        # """This is AI Generated, I will replace it with Qwen original code"""
        # if not hasattr(self, 'point_projection') or feature_dim != qwen_embed_dim:
        #     self.point_projection = nn.Linear(feature_dim, qwen_embed_dim).to(device).to(torch.float16)
        #     # Initialize with small weights
        #     nn.init.normal_(self.point_projection.weight, std=0.02)
        #     nn.init.zeros_(self.point_projection.bias)
        # point_embeddings = self.point_projection(feature_cloud.to(torch.float16))
        # Project point features to Qwen embedding space
        point_embeddings = feature_cloud.to(torch.float16)
        # Concatenate text embeddings and point embeddings
        # Shape: [batch_size, text_length + num_points, embed_dim]
        combined_embeds = torch.cat([text_embeds, point_embeddings], dim=1)
        
        # We'll need to bypass the input preparation in the model's forward method
        # and directly call the model's decoder
        
        # Create attention mask in the format expected by Qwen
        # (converting from 1=attend, 0=mask to 0=attend, -10000=mask)
        attn_mask = (1.0 - combined_mask) * -10000.0
        ...
        # Extend position IDs for point cloud tokens by using the last text token + 1
        ...
        # Use Qwen's internals to do the forward pass
    ...
    def forward(self, batched_inputs):
        ...
        attention_mask = self.build_knn_attention_mask(pointcloud, k=32)
        # torch.cuda.empty_cache()  # Clear cache after heavy 3D processing
        ...
        #     )
            ...
            pointcloud, featurecloud = self.process_pointcloud_in_chunks(
                pointcloud, featurecloud, attention_mask, target_sentence, chunk_size=chunk_size
            )
