...
class BasicLayer(nn.Module):
    ...
    def __init__(
        ...
        if cfg.MODEL.CROSS_VIEW_BACKBONE and cfg.MODEL.CROSS_VIEW_CONTEXTUALIZE and (
            cfg.MODEL.DECODER_3D or self.cfg.PASS_2D_CROSS_VIEW
        ):
            ...
            if layer_i == 0:
                ...
                self.cross_view_attn = CrossViewPAnet(
                    ...
                    num_layers=self.cfg.MODEL.CROSS_VIEW_NUM_LAYERS[layer_i],
                    cfg=cfg,
                )
    ...
    def forward(
        self, x, H, W, x_xyz=None, shape=None, multiview_data=None, decoder_3d=False
    ):
        """Forward function.
        Args:
            x: Input feature, tensor size (B, H*W, C).
        ...
        if (
            ...
            and self.cfg.MODEL.CROSS_VIEW_BACKBONE and self.cfg.MODEL.CROSS_VIEW_CONTEXTUALIZE
            and self.cross_view_attention
        ):
            # shortcut_x = x
...
class SwinTransformer(nn.Module):
    ...
    def forward(self, x, x_xyz=None, shape=None, multiview_data=None, decoder_3d=False):
        ...
        for i in range(self.num_layers):
            ...
            if self.cfg.MODEL.CROSS_VIEW_BACKBONE and (decoder_3d or self.cfg.PASS_2D_CROSS_VIEW):
                mv_data = {}
                mv_data["multi_scale_p2v"] = [
                    multiview_data["multi_scale_p2v"][self.xyz_dict[f"res{i+2}"]]
                ]
