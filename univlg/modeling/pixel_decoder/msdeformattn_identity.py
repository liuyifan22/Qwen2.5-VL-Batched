...
class MSDeformAttnPixelDecoderIdentity(nn.Module):
    @configurable
    def __init__(
        self,
    ...
    def forward_features(
        ...
        if decoder_3d and self.cfg.USE_GHOST_POINTS and not (not actual_decoder_3d and self.cfg.TREAT_FORCE_DECODER_3D_AS_2D):
            # interpolate features for skip connections
            f = self.in_features[-1]

            feats = interpolate_feats_3d(
                features[f],
                mask_features_xyz,
                mask_features_p2v,
                scannet_pc,
            ...
            for idx, f in enumerate(self.in_features[: self.num_fpn_levels][::-1]):
                ...
                if (decoder_3d and self.cfg.DO_TRILINEAR_INTERPOLATION) and not (not actual_decoder_3d and self.cfg.TREAT_FORCE_DECODER_3D_AS_2D):
                    ...
                    interp = interpolate_feats_3d(
                        out[-1],
                        multiview_data["multi_scale_xyz"][len(out) - 1],
                        multiview_data["multi_scale_p2v"][len(out) - 1]
